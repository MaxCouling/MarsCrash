...
# starts the pygame clock, helps with keeping the uprate at 60
...
class Rocketgame:
    ...
    def rocketGameRunning(self):
        ...
        while True:  # loops the game
            ...
            # this function is what makes the backround image move down in the
            # rocket videogame
            screen.blit(BG, (0, self.minigame_bgY))
            ...
            # self.rocket_list.draw(screen)#draws the rocket in the new x or y
            # depending on if the player has pressed a new input
            # blitting the rocket on the
            screen.blit(
                self.rocket.image,
                (self.rocket.rect.x,
                 self.rocket.rect.y))
            ...
            # asteroid collison code, if any of the asteroids in the astroid
            # lists hitbox hits the rockets hit box damage += 1
            ...
            for asteroid in asteroid_list:
                ...
                # gets the absolute speed, doesn't care for negatives
                asteroid.y -= abs(asteroid.speed * 10)
                asteroid.draw()
            ...
            pygame.draw.rect(
                ...
                pygame.Rect(
                    ...
                    HEALTHBAR_HEIGHT,
                    WINDOW_WIDTH,
                    HEALTHBAR_HEIGHT))  # the black behind the healthbar to show the player how much damage he has lost
            pygame.draw.rect(
                screen,
                RED,
                pygame.Rect(
                    ...
                    WINDOW_WIDTH -
                    length,
                    HEALTHBAR_HEIGHT))  # this is healthbar code (X , Y , WIDTH, HEIGHT),
            ...
            if self.rocket.rect.y <= 0:  # boundries in the game for y axis
                ...
                # makes the player cannot go past 368. Player image is 32px and
                # width is 400, 400 - 32 = 368.
                self.rocket.rect.y = WINDOW_HEIGHT - self.rocket.image.get_height() - \
                    HEALTHBAR_HEIGHT

            # making the game run at 60fps by limiting the amount of.0
            clock.tick(60)
    ...
    def mini_title_screen(self):
        ...
        while True:
            screen.fill(BLACK)
            # title will be a png saying press any key to continue
            screen.blit(self.title, (0, 0))
            screen.blit(
                pygame.font.Font.render(
                    Font, "PRESS ANY KEY TO CONTINUE", 1, WHITE), (70, 250))
            pygame.display.update()
